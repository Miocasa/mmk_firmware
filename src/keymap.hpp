...
inline void DEFAULT_LAYER_TAP_FUNC(TapDanceState *s, void *) {
    if (s->count == 1) {                    // tap или hold
        s->resolved_kc = KC_TRNS;
    }
    else if (s->count == 2 && !s->pressed) { // double tap
        s->resolved_kc = TO(KRITA);
    }
    else if (s->count == 2 && s->pressed) { // double tap + hold
...
constexpr uint16_t PROGMEM keymaps[LAYER_COUNT][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [KRITA] = LAYOUT(                   // Krita — цифровая живопись
        ...
        TD(KRITA_LAYER_TAP_DANCE), MO(KRITA_SECOND), LCTL(KC_S) // Save
    ),

    [KRITA_SECOND] = LAYOUT(            // Второй слой Krita — работа со слоями
...
constexpr EncoderMap PROGMEM encoderMaps[LAYER_COUNT] = {
    [MEDIA]       = { HID_USAGE_CONSUMER_VOLUME_INCREMENT,
                        HID_USAGE_CONSUMER_VOLUME_DECREMENT,
                        HID_USAGE_CONSUMER_PLAY_PAUSE       },

    [KRITA]         = { LCTL(KC_PLUS),   // rotate canvas right
                        LCTL(KC_MINUS),    // rotate canvas left
                        KC_TRNS          },

    [KRITA_SECOND]  = { LCTL(KC_RBRC),      // undo
                        LCTL(KC_LBRC),        // redo
                        KC_TRNS          },
